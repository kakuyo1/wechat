    ...
    , _current_ChosenTag_point(5, 5)
{
    ...
    // 显示更多标签
    connect(ui->moretags_icon, &TagLabel::clicked, this, &FriendRequestDialog::slot_showMoreTags);
    ...
    // 连接信号槽
    connect(ui->confirm_btn, &Customized_button::clicked, this, &FriendRequestDialog::slot_confirmBtn_clicked);
    connect(ui->cancel_btn, &Customized_button::clicked, this, &FriendRequestDialog::slot_cancelBtn_clicked);
    connect(ui->tags_lineEdit, &Customized_EditLine::returnPressed, this, &FriendRequestDialog::slot_enterKeyPressed);
    connect(ui->tags_lineEdit, &Customized_EditLine::textChanged, this, &FriendRequestDialog::slot_tagLineEdit_textChanged);
    connect(ui->tags_lineEdit, &Customized_EditLine::editingFinished, this, &FriendRequestDialog::slot_tagLineEdit_editFinished);
    connect(ui->tip_label, &TagLabel::clicked, this, &FriendRequestDialog::slot_tipLabel_clicked);
}
...
bool FriendRequestDialog::eventFilter(QObject *watched, QEvent *event)
{
...
void FriendRequestDialog::initTagLabels()
{
    ...
    for (const auto& tagText : _default_tag_texts) {
        ...
        connect(label, &Customized_Label::clicked, this, &FriendRequestDialog::slot_checkIfTagShallAdd);
...
void FriendRequestDialog::addTagToWidget(Customized_Label *tagLabel, QPoint currentPoint, QPoint &nextPoint, int textWidth, int textHeight)
{
    Q_UNUSED(textHeight);
...
void FriendRequestDialog::addToChosenTags(const QString &tagText)
{
    // 检查标签是否已经存在于已选择标签中
    ...
    // 监听移除标签的信号
    connect(chosenTag, &ChosenTag::signal_removeTag, this, &FriendRequestDialog::removeFromChosenTags);
...
void FriendRequestDialog::rearrangeChosenTags()
{
    // 从初始位置开始重新布局已选择标签
...
void FriendRequestDialog::addTagToallTagLabelsAndUpdateUI(const QString &text)
{
    // 1. 检查是否已存在该标签
    ...
    // 监听用户是否点击标签，从而决定是否添加到已选标签
    connect(newTagLabel, &Customized_Label::clicked, this, &FriendRequestDialog::slot_checkIfTagShallAdd);
    // 3. 计算标签尺寸
...
void FriendRequestDialog::relayoutAllTags()
{
...
void FriendRequestDialog::slot_showMoreTags()
{
    // 显示更多按钮隐藏
...
void FriendRequestDialog::slot_checkIfTagShallAdd()
{
...
void FriendRequestDialog::slot_enterKeyPressed()
{
...
void FriendRequestDialog::slot_tagLineEdit_textChanged(const QString &text)
{
    if (text.isEmpty()) {
...
void FriendRequestDialog::slot_tagLineEdit_editFinished()
{
    ui->input_tip_widget->hide(); // 编辑完成后隐藏输入提示
}

void FriendRequestDialog::slot_tipLabel_clicked()
{
