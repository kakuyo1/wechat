    ...
    _stopped(false)
{
    ...
    try{
        ...
        spdlog::error("Error initializing MySQL connection pool: {}", e.what());
...
void MysqlConnectionPool::ReturnConnection(std::unique_ptr<MysqlConnection> conn) {
    ...
    if (_connections.size() >= _pool_size) {
        spdlog::warn("Connection pool is full, discarding connection");
        return;
    }
    if (_stopped.load()) {
        return;
...
bool MysqlDAO::CheckEmailAndPasswordMatch(const std::string& email, const std::string& password, UserInfo& user_info) {
    ...
    try {
        ...
        std::unique_ptr<sql::PreparedStatement> stmt(con->con_->prepareStatement("SELECT * FROM user WHERE email = ? AND password = ?"));
        stmt->setString(1, email);
        stmt->setString(2, password);
        std::unique_ptr<sql::ResultSet> result(stmt->executeQuery());
        if (result->next()) {
            // use bcypt to check the password
            std::string stored_password = result->getString("password");
            if (bcrypt_checkpw(password.c_str(), stored_password.c_str()) == 0) {
                // Password matches, retrieve user info
        ...
        // If no result is returned, assume email and password do not match
        return false;
    } catch (sql::SQLException& e) {
        if (con) {
            _pool->ReturnConnection(std::move(con));
