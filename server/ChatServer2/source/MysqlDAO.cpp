...
bool MysqlDAO::RemoveItemFromFriendRequestList(int from_uid, int to_uid)
{
    /* remove the item from friend request list, if not exists, do nothing*/
    auto con = _pool->GetConnection();
    try {
        if (con == nullptr) {
            return false; // Connection failed
        }
        std::unique_ptr<sql::PreparedStatement> stmt(con->con_->prepareStatement("DELETE FROM friendrequestlist WHERE from_uid = ? AND to_uid = ?"));
        stmt->setInt(1, from_uid);
        stmt->setInt(2, to_uid);
        int result = stmt->executeUpdate();
        _pool->ReturnConnection(std::move(con));
